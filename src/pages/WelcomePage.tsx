...
const FEATURES = [
  {
    ...
    title: 'VEE Smart Picks',
    description: 'AI that learns what you love and curates your feed',
  },
  {
    icon: <Play className="w-6 h-6 text-accent" />,
    title: 'YouTube Trailers',
    description: 'Preview any movie before you commit',
  },
  {
    icon: <Wifi className="w-6 h-6 text-accent-green" />,
    title: 'Works on 3G',
    description: 'Eco mode for any connection speed',
  },
];
    ...
    <div className="min-h-screen bg-[#050508] text-white overflow-x-hidden">
      ...
      <header className="relative z-20 flex items-center justify-between px-5 py-4 sm:px-8 sm:py-5">
        ...
        <button
          ...
          className="px-4 py-2 rounded-xl text-sm font-medium border border-white/10 hover:border-primary/40 hover:bg-white/5 transition-all"
        >
          Sign In
        </button>
      </header>
        ...
        <div className="animate-fade-in-up">
          ...
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/10 mb-6">
            ...
            <span className="text-xs font-medium text-text-secondary tracking-wide">
              Showmax shut down. We didn't.
            </span>
          </div>

          {/* Headline */}
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold leading-[1.1] mb-5">
            <span className="text-white">Showmax is gone.</span>
            <br />
            <span className="text-gradient">Your streaming isn't.</span>
          </h1>

          {/* Subheadline */}
          <p className="text-lg sm:text-xl text-text-secondary max-w-xl mb-8 leading-relaxed">
            60,000+ movies. 11,000 live channels. YouTube trailers. AI-powered picks.{' '}
            <span className="text-white font-semibold">From $3/month.</span>
          </p>

          {/* CTA */}
          <button
            onClick={() => setShowLogin(true)}
            className="btn-sweep group flex items-center gap-3 px-7 py-4 rounded-2xl font-bold text-base tracking-wide transition-all duration-300 hover:-translate-y-0.5 text-white"
            style={{
              background: 'linear-gradient(135deg, #9D4EDD 0%, #7B2CBF 100%)',
      ...
      <section className="relative z-10 px-5 sm:px-8 pb-16 sm:pb-20 max-w-4xl mx-auto">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {FEATURES.map((f, i) => (
            <div
              key={i}
              className="rounded-2xl p-6 glass-light border border-white/[0.06] hover:border-primary/20 transition-all duration-300 hover:bg-white/[0.04] group"
      ...
      <section className="relative z-10 px-5 sm:px-8 pb-16 sm:pb-20 max-w-4xl mx-auto">
        <div className="section-glow pt-6">
          <h2 className="text-2xl sm:text-3xl font-bold mb-2">What's streaming right now</h2>
          <p className="text-text-secondary text-sm mb-8">Movies, series, live TV — all in one place</p>

          <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
            {GENRES.map((genre, i) => (
              <div
                key={genre}
                className={`aspect-[2/3] rounded-xl overflow-hidden bg-gradient-to-br ${GENRE_GRADIENTS[i]} border border-white/[0.08] flex flex-col items-center justify-center p-3 group hover:scale-[1.04] hover:border-primary/30 transition-all duration-300 cursor-pointer`}
                onClick={() => setShowLogin(true)}
              >
      ...
      <section className="relative z-10 px-5 sm:px-8 pb-16 sm:pb-24 max-w-4xl mx-auto">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-accent-green/10 border border-accent-green/20 mb-5">
            ...
            <span className="text-xs font-semibold text-accent-green">First 48 hours free</span>
          </div>
          ...
          <p className="text-text-secondary text-base mb-6 max-w-md mx-auto">
            Orange Money &bull; Cancel anytime &bull; Works on any device
          </p>

          <button
            onClick={() => setShowLogin(true)}
            className="btn-sweep inline-flex items-center gap-2 px-8 py-3.5 rounded-2xl font-bold text-sm tracking-wide transition-all duration-300 hover:-translate-y-0.5 text-white"
            style={{
              background: 'linear-gradient(135deg, #9D4EDD 0%, #7B2CBF 100%)',
      ...
      <footer className="relative z-10 border-t border-white/[0.06] px-5 sm:px-8 py-8">
        <div className="max-w-4xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-xs text-text-muted">
            Powered by <span className="text-text-secondary font-medium">DASH Lifestyle</span>
          </p>
          <nav className="flex items-center gap-6">
            {['Home', 'Live TV', 'Movies', 'Series'].map((label) => (
              <button
                key={label}
                onClick={() => setShowLogin(true)}
        ...
        <div className="fixed inset-0 z-[10000] flex items-center justify-center">
          ...
          >
            ...
            <div className="text-center mb-8">
              ...
              <p className="text-text-secondary text-sm">Enter your trial code to start streaming</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="relative">
                <input
                  ...
                  autoFocus
                  className="w-full py-[16px] px-5 pr-12 bg-white/[0.03] border border-white/[0.08] rounded-[14px] text-white text-base outline-none transition-all duration-300 placeholder:text-white/35 focus:border-primary focus:bg-white/[0.06] focus:shadow-[0_0_0_4px_rgba(157,78,221,0.15),0_0_20px_rgba(157,78,221,0.2)]"
                />
                <button
                  type="button"
              ...
              <button
                type="submit"
                disabled={loading}
                className="btn-sweep w-full py-[16px] rounded-[14px] font-bold text-sm tracking-[1.5px] uppercase transition-all duration-300 hover:-translate-y-0.5 disabled:opacity-60 disabled:cursor-not-allowed disabled:transform-none text-white"
                style={{
                  background: 'linear-gradient(135deg, #9D4EDD 0%, #7B2CBF 100%)',
            ...
            <p className="mt-6 text-text-muted text-xs text-center">
              48-hour free trial &bull; No credit card required
            </p>
          </div>
        </div>
      )}
